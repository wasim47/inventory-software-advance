...
class Ouradminmanage extends CI_Controller {
	...
	function index()
	{
		...
		$data['title']="dot3bd | Inventory Management System";
...
/////////////////////// Admin Part ////////////////////////////////	 
	...
	function company_info_list()
	{
		...
		$data['title']="company_info Shop List | dot3bd";
		$data['company_infolist'] = $this->Index_model->getTable('company_info','user_id','desc');
		$data['main_content']="admin/company_info/company_info_list";
        $this->load->view('admin_template',$data);
	} 
	 
	 
	 
	function company_info_registration()
	{
		if(!$this->session->userdata('AdminAccessMail')) redirect("ouradminmanage");
		$artiId=$this->uri->segment(3);
		$data['title']="New company_info Shop | dot3bdbd";
		if($artiId!=''){
		$original_value = $this->db->query("SELECT urlname FROM company_info WHERE user_id = ".$artiId)->row()->urlname;
			if($this->input->post('urlname') != $original_value) {
			   $is_unique =  '|is_unique[company_info.urlname]';
			} else {
			   $is_unique =  '';
			}
		}
		...
		$data['company_infoUpdate'] = $this->Index_model->getAllItemTable('company_info','user_id',$artiId,'','','user_id','desc');
		if($this->input->post('registration') && $this->input->post('registration')!=""){
			
		$this->form_validation->set_rules('company_info_name','company_infoshop Name','required|trim');
		if($this->form_validation->run() != false){
			$config['allowed_types'] = '*';
			$config['remove_spaces'] = true;
			$config['max_size'] = '1000000';
			$config['upload_path'] = './uploads/images/company_info/';
			$config['charset'] = "UTF-8";
			$new_name = "company_info_".time();
			...
			$save['active']	    = $this->input->post('status');
				...
				if($this->input->post('company_info_id')!=""){
					$b_id=$this->input->post('company_info_id');
					$query = $this->Index_model->update_table('company_info','user_id',$b_id,$save);
					$s='Updated';
				}
				else{
					$save['urlname']	    = $this->input->post('urlname');
				...
				class ".$urlname." extends CI_Controller { ";
					...
					//$password=$this->input->post('password');
							$tomaila=$email;
							$frommaila="info@dot3bdbd.com";
							$subjecta="Thank ".$this->input->post('company_info_name')." for registration with dot3bdbd.com";
							$config = array (
										  'mailtype' => 'html',
										  'charset'  => 'utf-8',
										  'priority' => '1'
                            	...
                            	<table width='100%' border='0' cellpadding='0' cellspacing='0' style='color:#fff;font-size:18px; background:#F7C11D'>
                                    ...
                                    <tr>
                                      ...
                                      <td height='49' colspan='3' align='center'>
                                        <strong>Your shop will be create within 24 hours.<br>If you have any query please contact dot3bdbd.com
                                                Support Team by email: info@dot3bdbd.com or mobile : +880673215210</strong></td>
                                      </tr>
                                       <tr>
                                      <td colspan='3'>&nbsp;</td>
                                      </tr>
							...
							//$this->email->initialize($config);
							$this->email->from($frommaila, 'dot3bdbd.com');
							...
							$tomail="info@dot3bdbd.com,wasim.html@gmail.com";
							$frommail=$email;
							$subject="New company_info shop request fro ".$this->input->post('company_info_name');
							$config = array (
										  'mailtype' => 'html',
										  'charset'  => 'utf-8',
										  'priority' => '1'
							...
							//$this->email->initialize($config);
							$this->email->from($frommail, 'dot3bdbd.com');
						...
						redirect('ouradminmanage/company_info_list', 'refresh');
					//}
				}
			}
			else{
				$data['main_content']="admin/company_info/company_info_action";
        		$this->load->view('admin_template', $data);
				}
		}
		else{
	...
	function purchase()
	{
		...
		$data['title']		= "Dashboard dot3bd | inventory";
		$data['inv_no']		= $this->Stock_model->purchaseinvoice();
		$data['supplier']	= $this->Stock_model->supplier();
		$data['product']	= $this->Stock_model->allp();
	...
	function purchasestock()
	{
		...
		/*$data['product_list']	= $this->Stock_model->allp();*/
	...
	public function purchaseinvoice()
	{
		...
		$this->Stock_model->invoice_submit($invNO,$mainDate,$mainDeliDate,$supplier,$net_total,$pro_code,$pro_name,$pro_id,$qty,$price,$net,$minvoice);
	...
	{	
	if(!$this->session->userdata('AdminAccessMail')) redirect("ouradminmanage");
		$update['hs_pid']=$this->input->post('product_id');
		
			$query = $this->db->query("select * from stock where hs_pid ='".$update['hs_pid']."'");
			if($query->num_rows() > 0){
		...
		$return = $this->input->post('return');
			...
			if(isset($add) && $add=='Add'){
				...
				$update['hs_qty']=$qty + $this->input->post('pluse_qty');
			...
			elseif(isset($minus) && $minus=='Minus'){
				...
				$update['hs_qty']=$qty - $this->input->post('minus_qty');
				$update['date']=date('Y-m-d');
				
				$save['hs_pid']=$this->input->post('product_id');
				...
				$save['hs_qty']=$this->input->post('minus_qty');
			...
			elseif(isset($return) && $return=='Return'){
				...
				$update['hs_qty']=$qty + $this->input->post('return_qty');
				$update['date']=date('Y-m-d');
				...
				$save['buyercontact']=$this->input->post('buyercontact');
				$save['buyeremail']=$this->input->post('buyeremail');
				$save['hs_qty']=$this->input->post('return_qty');
				$save['remarks']=$this->input->post('remarks');
				$save['ret_date']=date('Y-m-d');
				$status = 'return';
			}
		$this->Index_model->stock_update($update,$save,$status); 
		redirect('ouradminmanage/purchasestock', '');
	}
	...
	{
		if(!$this->session->userdata('AdminAccessMail')) redirect("ouradminmanage");
		$data['title']="Article List dot3bd | inventory";
	...
	{
		if(!$this->session->userdata('AdminAccessMail')) redirect("ouradminmanage");
		$data['title']="Admin Registration | dot3bd";
	...
	{
		if(!$this->session->userdata('AdminAccessMail')) redirect("ouradminmanage");
		$data['title']="customer List | dot3bd";
	...
	function customer_registration()
	{
		...
		$data['title']="Customer Registration | dot3bd";
...
public function menu_check($str,$boutique)
	...
	{
		if(!$this->session->userdata('AdminAccessMail')) redirect("ouradminmanage");
		$data['title']="Article List | dot3bd";
	...
	function article_registration()
	{
		...
		$artiId=$this->uri->segment(3);
		if(!$artiId){
			$data['title']="Article Registration | dot3bd";
		}
		else{
			$data['title']="Article Update | dot3bd";
		}
	...
	{
		if(!$this->session->userdata('AdminAccessMail')) redirect("ouradminmanage");
		$data['title']="feature List | dot3bd";
	...
	function feature_registration()
	{
		...
		$artiId=$this->uri->segment(3);
		if(!$artiId){
			$data['title']="feature Registration | dot3bd";
		}
		else{
			$data['title']="feature Update | dot3bd";
		}
...
/////////////////////// Class ////////////////////////////////	 
	...
	{
		if(!$this->session->userdata('AdminAccessMail')) redirect("ouradminmanage");
		$data['title']="Category List | dot3bd";
	...
	function class_registration()
	{
		...
		$data['title']="Classes List | dot3bd";
...
/////////////////////// sub_category ////////////////////////////////	 
	...
	{
		if(!$this->session->userdata('AdminAccessMail')) redirect("ouradminmanage");
		$data['title']="sub_category List | dot3bd";
	...
	function sub_category_registration()
	{
		...
		$data['category_list'] = $this->Index_model->getTable('category','cid','desc');
		if(!$artiId){
			$data['title']="sub_category Registration | dot3bd";
			...
			$data['title']="sub_category Update | dot3bd";
	...
	{
		if(!$this->session->userdata('AdminAccessMail')) redirect("ouradminmanage");
		$data['title']="last_category List | dot3bd";
	...
	function last_category_registration()
	{
		...
		if(!$artiId){
			$data['title']="last_category Registration | dot3bd";
			...
			$data['title']="last_category Update | dot3bd";
			$this->form_validation->set_rules('last_category_name', 'last_category name', 'trim|required');
		}
		if($this->input->post('registration') && $this->input->post('registration')!=""){
			if($this->form_validation->run() != false){
	...
	if(!$this->session->userdata('AdminAccessMail')) redirect("ouradminmanage");	
		$artiId=$this->uri->segment(3);
		if(!$artiId){
			$data['title']="Banner Registration | dot3bd";
		}
		else{
			$data['title']="Banner Update | dot3bd";
		}
...
/////////////////////// advertisement ////////////////////////////////	 
	...
	{
		if(!$this->session->userdata('AdminAccessMail')) redirect("ouradminmanage");
		$data['title']="advertisement List | dot3bd";
	...
	function advertisement_registration()
	{
		...
		$artiId=$this->uri->segment(3);
		if(!$artiId){
			$data['title']="advertisement Registration | dot3bd";
		}
		else{
			$data['title']="advertisement Update | dot3bd";
		}
	...
	{
		if(!$this->session->userdata('AdminAccessMail')) redirect("ouradminmanage");
		$data['title']="supplier Shop List | dot3bd";
	...
	function supplier_registration()
	{
		if(!$this->session->userdata('AdminAccessMail')) redirect("ouradminmanage");
		$artiId=$this->uri->segment(3);
		$data['title']="New supplier Shop | dot3bdbd";
		if($artiId!=''){
		...
		$this->form_validation->set_rules('supplier_name','suppliershop Name','required|trim');
		if($this->form_validation->run() != false){
			$config['allowed_types'] = '*';
			$config['remove_spaces'] = true;
			$config['max_size'] = '1000000';
			...
			$save['active']	    = $this->input->post('status');
				...
				class ".$urlname." extends CI_Controller { ";
					...
					//$password=$this->input->post('password');
							$tomaila=$email;
							$frommaila="info@dot3bdbd.com";
							$subjecta="Thank ".$this->input->post('supplier_name')." for registration with dot3bdbd.com";
							$config = array (
										  'mailtype' => 'html',
										  'charset'  => 'utf-8',
										  'priority' => '1'
							...
							$tomail="info@dot3bdbd.com,wasim.html@gmail.com";
							$frommail=$email;
							$subject="New supplier shop request fro ".$this->input->post('supplier_name');
							$config = array (
										  'mailtype' => 'html',
										  'charset'  => 'utf-8',
										  'priority' => '1'
						...
						$this->session->set_flashdata('successMsg', '<h2 class="alert alert-success">Successfully '.$s.'</h2>');
						redirect('ouradminmanage/supplier_list', 'refresh');
					//}
				}
			}
			else{
	...
	function agent_registration()
	{
		$artiId=$this->uri->segment(3);
		if(!$artiId){
			$data['title']="New agent Shop | dot3bd";
		}
		else{
			$data['title']="Update agent Shop | dot3bd";
	...
	{
		if(!$this->session->userdata('AdminAccessMail')) redirect("ouradminmanage");
		$data['title']="Product List | dot3bd";
		$data['product_list'] = $this->Index_model->getTable('product','product_id','desc');
	...
	function product_registration()
	{
		...
		$artiId=$this->uri->segment(3);
		if(!$artiId){
			$data['title']="Product Insert | dot3bd";
		}
		else{
			$data['title']="Product Update | dot3bd";
		}
	...
	{
		if(!$this->session->userdata('AdminAccessMail')) redirect("ouradminmanage");
		$data['title']="photogallery List | dot3bd";
	...
	function photogallery_registration()
	{
		...
		$artiId=$this->uri->segment(3);
		if(!$artiId){
			$data['title']="photogallery Registration | dot3bd";
		}
		else{
			$data['title']="photogallery Update | dot3bd";
		}
...
function order_list()
	 {
		...
		$data['title']="dot3bd | Customer Order List";
...
/*	
	...
	function view_order($order_id)
	 {
		 if(!$this->session->userdata('AdminAccessMail')) redirect("ouradminmanage");
			...
			$data['status']=$status;
			$data['title']="dot3bd | Customer Order Details";
	...
	{
		if(!$this->session->userdata('AdminAccessMail')) redirect("ouradminmanage");
		$data['title']="dot3bd | Inventory Management";
	...
	{
		if(!$this->session->userdata('AdminAccessMail')) redirect("ouradminmanage");
		$data['title']="dot3bd | Inventory History";
		$data['pid']= $pid;
		...
		$genereteInvoice=array(
			...
			foreach($incsource->result() as $incrow);
			 ...
			 foreach($orderproid->result() as $orPro){
				...
				$prod_qty = $orPro->qty;
				$query = $this->db->query("select * from stock where hs_pid ='".$prod_id."'");
				if($query->num_rows() > 0){
					foreach($query->result() as $row);
					$qty = $row->hs_qty; 
				}
				...
				$update['hs_qty']=$qty - $prod_qty;
				$update['hs_pid']=$prod_id;
				$save['hs_pid']=$prod_id;
				$save['buy_type']="Retail";
				$save['cust_id']=$customer_id;
				$save['hs_qty']=$prod_qty;
	...
	{
		if(!$this->session->userdata('AdminAccessMail')) redirect("ouradminmanage");
		$data['title']="asset_investment List | dot3bdbd";
	...
	function asset_investment_registration()
	{
		...
		$data['title']="asset_investment Registration | dot3bdbd";
	...
	{
		if(!$this->session->userdata('AdminAccessMail')) redirect("ouradminmanage");
		$data['title']="internal_cost List | dot3bdbd";
	...
	function internal_cost_print()
	{
		...
		$data['title']="Payment Print | dot3bdbd";
		if(!$printsegment){
	...
	function daily_sale_reports()
	{
		if(!$this->session->userdata('AdminAccessMail')) redirect("ouradminmanage");
		$printsegment=$this->uri->segment(3);
		$data['title']="Daily Sale Reports | dot3bdbd";
		$today=date('Y-m-d');
		$data['todayOrder'] = $this->Index_model->getAllItemTable('orders','date',$today,'status','Successfull Delivery','order_id','desc');
	...
	function datewise_sale_reports()
	{
		if(!$this->session->userdata('AdminAccessMail')) redirect("ouradminmanage");
		$printsegment=$this->uri->segment(3);
		$data['title']="Date Wise Sale Reports | dot3bdbd";
		if(!$printsegment){
	...
	function datewise_sale_reports_ajax()
	{
		...
		$data['datewisOrder'] = $this->db->query("select * from orders where (date between '$fromdate' and '$todate') and status='Successfull Delivery' order by order_id desc");
	...
	function today_reports()
	{
		if(!$this->session->userdata('AdminAccessMail')) redirect("ouradminmanage");
		$printsegment=$this->uri->segment(3);
		$data['title']="Today Reports | dot3bdbd";
		if(!$printsegment){
	...
	function datewise_reports()
	{
		if(!$this->session->userdata('AdminAccessMail')) redirect("ouradminmanage");
		$printsegment=$this->uri->segment(3);
		$data['title']="Date Wise Reports| dot3bdbd";
		if(!$printsegment){
	...
	function purchasereport()
	{
		if(!$this->session->userdata('AdminAccessMail')) redirect("ouradminmanage");
		$printsegment=$this->uri->segment(3);
		$data['title']="Purchase Invoice Reports| dot3bdbd";
		if(!$printsegment){
			$data['supplier']	= $this->Stock_model->supplier();
	...
	function purchase_ajax()
	{
		...
		$data['fromdate']=$this->session->userdata('toDate');
		$data['todate']=$this->session->userdata('fromDate');
		$data['supplier']=$this->session->userdata('supplier');
	...
	function stockin_reports()
	{
		...
		$printsegment=$this->uri->segment(3);
		$data['supplier']		= $this->Index_model->getDataById('supplier','','','username','asc','');
		$data['title']="Current Stock Reports | dot3bdbd";
		if(!$printsegment){
	...
	function stockin_reports_ajax()
	{
		if(!$this->session->userdata('AdminAccessMail')) redirect("ouradminmanage");
			...
			if($this->input->get('key')!=""){
				...
				$arrPro = join(',',$proid);
				$sql=$this->db->query("select * from stock where hs_pid IN($arrPro) order by hs_id desc");
			}
			elseif($this->input->get('supplier')!=""){
				$keyword = $this->input->get('supplier');
				$sessiondata = array(
							'supplier'=>$keyword
						   );
				$this->session->set_userdata($sessiondata);
				$proKey=$this->session->userdata('supplier');
				
				$sqlPeo=$this->db->query("select * from product where supplier='$proKey' order by product_id desc");
				foreach($sqlPeo->result() as $pr){
					$proid[] = $pr->product_id;
				}
				$arrPro = join(',',$proid);
			...
			elseif($this->input->get('currentstock')!=""){
				$sql=$this->db->query("select * from stock order by hs_id desc");
			}
			elseif($this->input->get('tdate')!=""){
				...
				$fromdate=$this->session->userdata('fromDate');
				$todate=$this->session->userdata('toDate');
				$sql=$this->db->query("select * from stock where (date between '$fromdate' and '$todate') order by hs_id desc");
			}
